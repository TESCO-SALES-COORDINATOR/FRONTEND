...
const HandoverForm = ({ record, leads = [], isDuplicateLead, onCancel, onSaved }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.leadId) { addToast('Please select a Lead ID', 'error'); return; }
    // Only ONE order confirmation per lead — block a duplicate create with a toast.
    if (!isEdit && typeof isDuplicateLead === 'function' && isDuplicateLead(form.leadId)) {
      addToast('This lead already has an order confirmation. Only one is allowed per lead.', 'error');
      return;
    }
    if (!percentageValid) {
      addToast('Total payment percentage must be exactly 100%.', 'error');
      return;
    }
    setSaving(true);
    const id = isEdit ? record.id : form.leadId;
    const payload = {
      id,
      leadId: form.leadId,
      ...form,
    ...
    try {
      if (isEdit) {
        const res = await fetch(`${PROJECTS_API}/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        if (!res.ok) throw new Error('update failed');
        addToast('Handover updated', 'success');
      } else {
        const res = await fetch(PROJECTS_API, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        if (!res.ok) throw new Error('create failed');
        addToast('Handover submitted', 'success');
      }
      ...
      if (lead && !/payment collection|completed/i.test(String(lead.status || ''))) {
        ...
        fetch(`${LEADS_API}/${form.leadId}`, {
          method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'Payment Collection', history })
        }).catch((err) => console.error('Failed to advance lead to Payment Collection:', err));
      }
      onSaved();
    } catch (err) {
      console.error('Failed to save handover form:', err);
      addToast('Could not save. Is the backend running on :5000?', 'error');
    } finally {
      setSaving(false);
    }
  };
