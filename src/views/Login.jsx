...
// ─────────────────────────────────────────────────────────────────────────────
// Sales COORDINATOR login. Same layout as the original Nexus CRM login, but the
// role is LOCKED to "Sales Coordinator": the "Select Role" dropdown is removed and
// the role is shown as a fixed default. Tinted teal so it's distinct from the
// Manager (indigo) and Head (amber) portals. Auth/OTP/validation are unchanged.
// ─────────────────────────────────────────────────────────────────────────────
const APP_ROLE = 'Sales Coordinator';
const ACCENT = '#4F46E5';        // indigo/navy (original Nexus CRM accent)
...
const Login = () => {
  ...
  // Inline feedback (no external toast provider needed)
  ...
  useEffect(() => {
    ...
    if (params.get('loggedout') === '1') clearSession();
  }, []);
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      // Role is fixed by the application — never taken from user input.
      const data = await authApi.login(APP_ROLE, email, password);
      // Defense-in-depth: reject a token whose account is not a Coordinator.
      if (data.user?.role && data.user.role !== APP_ROLE) {
        clearSession();
        setBlockedMsg('This portal is for Sales Coordinators only. Please use your role’s application.');
        return;
      }
    ...
    <div className="login-page-wrapper" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#F8FAFC', fontFamily: "'Inter', sans-serif" }}>
        ...
        <div className="login-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2.5rem', width: '100%', alignItems: 'center' }}>
          ...
          <div className="left-hero-card" style={{ background: `linear-gradient(135deg, ${HERO_FROM} 0%, ${HERO_TO} 100%)`, border: `1px solid ${HERO_BORDER}`, borderRadius: '1.5rem', padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.25rem', boxShadow: '0 4px 20px -2px rgba(191,219,254,0.3)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <h1 className="hero-title" style={{ fontSize: '2.25rem', fontWeight: 800, color: '#1E1B4B', lineHeight: 1.2, letterSpacing: '-1px', margin: 0 }}>Coordinate Sales<br />Workflow Smarter</h1>
              ...
              {forgotStep === 0 && (
                <>
                  ...
                  <form onSubmit={handleLoginSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                    {/* Role (fixed default — not selectable) */}
                    <div>
      ...
      <div style={{ borderTop: '1px solid #E2E8F0', padding: '1.5rem', backgroundColor: '#FFFFFF' }}>
        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', maxWidth: '1280px', margin: '0 auto', width: '100%', fontSize: '0.75rem', color: '#64748B', flexWrap: 'wrap', gap: '1rem' }}>
          <span>&copy; 2026 Nexus CRM · Sales Coordinator. All rights reserved.</span>
